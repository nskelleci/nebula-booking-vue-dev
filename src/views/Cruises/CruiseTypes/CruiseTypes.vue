...
<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <cruisetype-form :isSidebarActive="addNewDataSidebar" @closeSidebar="toggleDataSidebar" :data="sidebarData" />

    <vs-table ref="table"   v-model="selected" pagination :max-items="itemsPerPage" search :data="cruisetypes">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4">
          <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ cruisetypes.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : cruisetypes.length }} of {{ queriedItems }}</span>
            <feather-icon icon="ChevronDownIcon" svgClasses="h-4 w-4" />
          </div>
        ...
        <tbody>
          <vs-tr :data="cruisetype" :key="_id" v-for="(cruisetype,_id) in data" >
            ...
            <vs-td class="whitespace-no-wrap">
              ...
              <feather-icon icon="EditIcon" svgClasses="w-7 h-7 hover:text-primary stroke-current" class="ml-4" @click.stop="editData(cruisetype)" />
              <feather-icon icon="TrashIcon" svgClasses="w-7 h-7 hover:text-danger stroke-current" class="ml-4" @click.stop="deleteData(cruisetype._id)" />
            </vs-td>
          </vs-tr>
        </tbody>
      </template>
...
import CruisetypeForm  from './CruiseTypeForm'
