...
<template>
  <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
    ...
    <div class="p-6">
        ...
        <vs-input label="Password" v-model="password" v-show="Object.entries(this.data).length === 0 ? true : false" class="mt-5 w-full" name="agency-password" v-validate="'required|min:6'" />
        <span class="text-danger text-sm" v-show="Object.entries(this.data).length === 0 ? true : false">{{ errors.first('agency-password') }}</span>
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          if (this.dataId !== null && this.dataId.length >= 0) {
            this.$store.dispatch('updateAgency', objCreate).catch(err => { console.error(err) })
